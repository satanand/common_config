...
if !exists("g:winManagerWindowLayout")
   ...
   "let g:winManagerWindowLayout = "
   "FileExplorer,TagList|BufExplorer"
end
	...
	" otherwise goto the last listed buffer being edited.
		...
		if bufwinnr(lastBufferNumber) != -1
		    ...
		    elseif (&modified && !&hidden)
				   exe 'silent! vsplit '.bufcall
				   exe &columns-g:winManagerWidth.' wincmd |'
				"call s:ResizeNonExpWindowsOpen()
			    "call s:GotoWindow(bufwinnr(bufnr(lastBufferNumber)))
				"exe &columns-g:winManagerWidth.' wincmd |'
			else
...
function! <SID>ResizeNonExpWindowsOpen()
	let i = 1
	let j = 1

	let vsp_resize_wid = 0
	" loop over all open windows
	while 1
		if winbufnr(i) == -1
			break
	...
	endwhile
	let vsp_resize_wid = vsp_resize_wid + (&columns-g:winManagerWidth)/i
	while 1
		if winbufnr(j) == -1
			break
		end
		if !s:IsExplorerBuffer(winbufnr(j))
			 call s:GotoWindow(winbufnr(j))
			 exe vsp_resize_wid.' wincmd |'
		     let j = j + 1
		end
	endwhile
	return 
